...
import { collection, addDoc, query, where, onSnapshot, updateDoc, doc, setDoc, serverTimestamp, getDocs } from 'firebase/firestore'
...
async function searchFood(q) {
  const url = `${FOOD_API}?search_terms=${encodeURIComponent(q)}&search_simple=1&action=process&json=1&page_size=8&fields=product_name,nutriments`
...
export default function NutritionTab({ profile, uid, nutritionDays, setNutritionDays, todayCalories, setTodayCalories }) {
  ...
  // Live food log listener - listen to all foodLog, filter by date client-side
  useEffect(() => {
    if (!uid) return
    const unsub = onSnapshot(collection(db, 'users', uid, 'foodLog'), snap => {
      ...
      console.error('Food log listener error:', err)
    })
    return () => unsub()
  }, [uid, todayStr])
  ...
  const saveFood = async (name, calories, protein) => {
    ...
    try {
      ...
      alert('Error saving food. Check your connection.')
    }
    setMode(null)
    ...
    <div style={{ padding: '0 20px 110px' }}>
      ...
      <Card style={{ marginBottom: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
          ...
          <span style={{ fontFamily: mono, fontSize: 12, color: MUT }}>{proteinLogged}g<span style={{ color: DIM }}>/{profile.protein || 150}g</span></span>
        </div>
        <div style={{ background: '#1a2a1a', borderRadius: 99, height: 8, overflow: 'hidden' }}>
          <div style={{ background: '#86efac', width: `${Math.min((proteinLogged / (profile.protein || 150)) * 100, 100)}%`, height: '100%', borderRadius: 99, transition: 'width 0.6s' }} />
        </div>
      </Card>
      ...
      {mode === null && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
          <button onClick={() => setMode('search')} style={{ background: G, color: '#000', border: 'none', borderRadius: 12, padding: '14px', fontFamily: bebas, fontSize: 17, letterSpacing: 1, cursor: 'pointer' }}>🔍 SEARCH</button>
          <button onClick={() => setMode('manual')} style={{ background: 'transparent', color: G, border: `1px solid ${BORD}`, borderRadius: 12, padding: '14px', fontFamily: bebas, fontSize: 17, letterSpacing: 1, cursor: 'pointer' }}>+ MANUAL</button>
        </div>
      )}
      ...
      {mode === 'search' && (
        <Card style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
            <input value={searchQ} onChange={e => setSearchQ(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} placeholder="e.g. chicken breast…"
      ...
      {log.length === 0
        ...
        : log.map((food, i) => (
          <Card key={food.id || i} style={{ marginBottom: 8 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
