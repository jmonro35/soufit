...
export default function Onboarding({ uid, onComplete }) {
  ...
  const handleRecover = async () => {
    ...
    try {
      ...
      const oldProfile = userSnap.data()
      // Copy old profile to current anonymous uid so Firebase auth matches
      await setDoc(doc(db, 'users', uid), { ...oldProfile, uid, onboarded: true })
      // Update group member record to new uid
      if (oldProfile.groupCode) {
        const oldMemberSnap = await getDoc(doc(db, 'groups', oldProfile.groupCode, 'members', userId))
        if (oldMemberSnap.exists()) {
          await setDoc(doc(db, 'groups', oldProfile.groupCode, 'members', uid), { ...oldMemberSnap.data(), uid })
        }
      }
      ...
      <div style={{ padding: '60px 24px 0' }}>
        ...
        <Card style={{ marginBottom: 16 }}>
          ...
          {[
            ...
            { icon: '💸', text: 'Miss a workout? $2 goes into the group pot' },
  ...
  if (step === 'venmo') return wrap(<>
    ...
    <Card style={{ marginBottom: 20 }}>
      ...
      {['Miss workout goal → $2','Nutrition goal — no penalty yet','Max $2/week per person','Month end → best record wins','Pot holder pays out the winner'].map((s,i) => (
